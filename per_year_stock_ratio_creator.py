...
def sendToOutput(peData):
    with open('yearlys&p/'+str(RANGE)+'_year/s&p500_pe_stock_ratio_'+str(yearConst)+'.csv', 'a', newline='\n') as outputAppendCsv:
...
for rangeConst in range(1, 6):
    ...
    for yearConst in range(2007, 2022-RANGE):
        with open('s&p500_name_code.csv', newline='\n') as csvfile:
            ...
            i = 0
            with open('yearlys&p/'+str(RANGE)+'_year/s&p500_pe_stock_ratio_'+str(yearConst)+'.csv', 'w', newline='\n') as outputCsv:
